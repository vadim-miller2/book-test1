...
-   Blobs have a limited "time-to-live" (TTL).
    ...
    -   Blob lifetime can be prolonged.
        ...
        -   Lifetime prolongation can be disabled by setting the '''prolong\_on\_read''' entry to <span class="nctnt ncbi-code">false</span> in [netcached.ini](http://www.ncbi.nlm.nih.gov/viewvc/v1/trunk/c++/src/app/netcache/netcached.ini?view=log).

        -   <span class="nctnt highlight">Note:</span> Calling <span class="nctnt ncbi-func">GetBlobSize()</span> will prolong a blob's lifetime (unless '''prolong\_on\_read''' is <span class="nctnt ncbi-code">false</span>), but calling <span class="nctnt ncbi-func">GetBlobInfo()</span> will not.
...
If you are using <span class="nctnt ncbi-class">CNetICacheClient</span>, you either need to use API methods that take a cache name or, to take advantage of automatic configuration based on the registry, specify a cache name in the '''[netcache\_api]''' section, for example:
